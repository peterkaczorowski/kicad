        ...
        m_height( aParent, m_HeightLabel, m_ModHeight, m_HeightUnit )
{
    ...
    m_imageEditor = new PANEL_IMAGE_EDITOR( aParent, m_PanelImage, aBitmap.GetReferenceImage().MutableImage() );
    ...
    // control column, above its PPI / Greyscale grid (matching the 9.0.6-imx layout:
    // image on the left, all size controls on the right).  The box is split into two
    // sub-columns: the left holds Width / Height / Lock aspect ratio, the right holds
    // the Units selector (aligned with the Width row).
    ...
    // Anchor box keeps its natural (~1/7) width; the stretch spacer creates the wide right margin.
    ...
    m_unitSelector->Bind( wxEVT_CHOICE, &DIALOG_REFERENCE_IMAGE_PROPERTIES::onUnitChanged, this );
...
bool DIALOG_REFERENCE_IMAGE_PROPERTIES::TransferDataToWindow()
{
    ...
    // The Position fields show the chosen anchor point of the bounding box (this is
    // also the value serialised as the image's (at ...)).
...
bool DIALOG_REFERENCE_IMAGE_PROPERTIES::TransferDataFromWindow()
{
    if( m_imageEditor->TransferDataFromWindow() )
    {
        ...
        // Apply the edited image (carrying its final per-axis scale) first, so the
        // bounding-box size is up to date before we pin the anchor position.
        m_imageEditor->TransferToImage( refImage.MutableImage() );

        // Set the anchor and the anchor-relative position.
...
bool DIALOG_REFERENCE_IMAGE_PROPERTIES::isPercentMode() const
{
    return m_unitSelector->GetSelection() == 3;
}


void DIALOG_REFERENCE_IMAGE_PROPERTIES::applyUnitSelection()
{
...
int DIALOG_REFERENCE_IMAGE_PROPERTIES::getAnchorSelection() const
{
    for( int i = 0; i < 9; ++i )
    {
...
void DIALOG_REFERENCE_IMAGE_PROPERTIES::setAnchorSelection( int aIndex )
{
...
void DIALOG_REFERENCE_IMAGE_PROPERTIES::setWidthField()
{
    if( isPercentMode() )
...
void DIALOG_REFERENCE_IMAGE_PROPERTIES::setHeightField()
{
    if( isPercentMode() )
...
void DIALOG_REFERENCE_IMAGE_PROPERTIES::updateSizeFields()
{
    setWidthField();
    setHeightField();
}
...
void DIALOG_REFERENCE_IMAGE_PROPERTIES::onUnitChanged( wxCommandEvent& aEvent )
{
    applyUnitSelection();
    updateSizeFields();
}
